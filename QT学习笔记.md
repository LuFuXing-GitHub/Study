...
数据库
    ...
    QString dbPath = QCoreApplication::applicationFilePath();
    m_db.setDatabaseName(dbPath + "../mydata.db");
	//这里是连接到数据库，需要数据库文件存在，但是sqlite则不需要，如果该数据库文件不存在则会则第一次连接时创建，这是仅属于sqlite的特性

```

```
Qt线程中不能使用QMassageBox类，也就是说线程里面不能操作ui
Qt中数据库句柄QsqlDataBase不能跨线程使用，无论是通过信号与槽传递、静态函数获取、都不行，最好的方法是将数据库的操作放在某个公告的类里面，不然就是子线程也同样连接到相同的数据库，两者不会冲突
```
...
问题：为什么不联网也可以正常通过主机名连接？可不可以通过？IP地址连接

​	为什么解析的主机名不一样？

```
如果出现解析的客户端主机名为“bogon”则代表网络有问题，一般是内网测试，必须主机和虚拟机之间就会出现这种情况
```

思路：

```
1.首先，客户端打开就自行进行连接到服务器端的操作，服务器端会相应的显示客户端的IP地址、主机号、端口号等信息
2.点击登录的时候发送账号和密码信息，服务器端通过遍历ui上的IP地址、主机号、端口号来对比从而添加账号信息
3.后续客户端发送消息的时候可直接遍历ui上的账号由此找到对应IP、主机号、端口号的socket来进行消息的转发


11.5
进度：客户端已经可以发送登录账号和密码信息通过加密的方式发送到服务器，服务器主线程也可以接收到线程解析的内容
1.客户端登录槽需改成登录成功后才记录信息到注册表
2.先完成注册功能，客户端逻辑编写，服务器数据库加进来
	注册界面已搭好，界面美化工作暂未完成，先完成实际逻辑业务
3.数据库的操作先看看书，然后再找找CSDN

11.6
进度：客户端登录 注册均可正常发送json格式的信息到服务器
1.服务器添加数据库模块完成以客户端所需
2.学习Qt发送验证码

11.7
进度
1.当前已可以操作数据库，需要创建数据表
2.设置日志的打印失败的为红色显示，成功或者其他打印则无变化
3.数据库句柄是在主线程，子线程是通过主线程调用子线程函数传给子线程的

11.11
进度
1.登录 注册 注销功能已完成
	注册和注销后续客户端、服务器端需添加验证码功能
2.
11.13
进度
1.主界面美化，尤其好友列表
...
进度
1.添加好友，添加需要等待对方确认
2.登录成功一个账户，服务器就存储一个 QMap<ID,threadID>,后续服务器的消息转发都通过遍历该容器进行，不过发之前需判断对方是否在线
3.构思消息存储问题，如果对方不在线，则消息如何存储？？？下次上线判断有没有来自其他用户的消息
11.14
进度
1.加入头像和昵称，账号登录时，发送的登录请求服务器收到后，在对比账号密码的同时还要判断是否设置了头像和昵称，如果有返回该账号的头像和昵称到客户端，客户端进行相应的显示，如果没有也返回到客户端，客户端会判断没有设置头像和昵称的话弹出设置头像和昵称页面
2.整个图片的存储流程：
	客户端设置头像的时候：1.上传该图片的源文件到服务器，一般是二进制格式QByteArray，2.服务器接收到该数据之后将其作为文件存储在资源管理器中，并将其账号与图片的文件路径存储到数据表中
